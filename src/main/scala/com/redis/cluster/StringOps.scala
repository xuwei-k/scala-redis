...
trait StringOps extends StringApi {
  ...
  override def set(key: Any, value: Any)(implicit format: Format): Boolean =
    processForKey(key)(_.set(key, value))

  override def set(key: Any, value: Any, onlyIfExists: Boolean, time: redis.SecondsOrMillis): Boolean =
    processForKey(key)(_.set(key, value, onlyIfExists, time))
